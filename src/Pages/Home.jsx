...
const categories = DataPosts.categories
...
.map((post) => ({
  ...
  authorIamge: post.author.avatar,
  date : post.date,
  slug : post.slug
}))
const recentArticle = posts.slice(3, 9).map((post) => ({
  title : post.title,
  Description : post.excerpt,
  category : post.category,
  readtime : post.readTime,
...
const stats = [
  { icon: <FaNewspaper />, value: `${posts.length}+`, label: "مقالة" },
  { icon: <FaUsers />, value: "+10ألف", label: "قارئ" },
  { icon: <FaFolderOpen />, value: categories.length.toString(), label: "تصنيفات" },
        ...
        <div className="text-center max-w-4xl mx-auto">
             <div className="inline-flex items-center gap-2 mb-8 animate-fade-in">
              ...
              <span className="text-sm font-medium text-neutral-300">
                مرحباً بك في {DataPosts.siteInfo.name}
              </span>
            </div>
     
            <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold text-white mb-6 leading-tight tracking-tight">
              اكتشف <span className="gradient-text text-orange-400">فن</span>
              <br />
              التصوير الفوتوغرافي
            </h1>

            <p className="text-xl md:text-2xl text-neutral-400 mb-10 max-w-2xl mx-auto leading-relaxed">
              {DataPosts.siteInfo.description}
            </p>
                     
            <div className="flex flex-col sm:flex-row justify-center gap-4 mb-16">
              <Link to="/blog">
                <button className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-linear-to-r from-orange-500 to-orange-600 text-white rounded-xl font-semibold transition-all duration-300 hover:shadow-lg hover:shadow-orange-500/50 hover:-translate-y-0.5 group">
...
<div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-3xl mx-auto">
  {stats.map((stat, index) => (
                ...
                <div
                  key={index}
                  className="glass-card bg-[rgba(22,22,22,0.5)] p-4 rounded-2xl hover:scale-105 transition-transform duration-300 flex flex-col items-center text-center"
                >
 ...
 <div className='relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-4">
            ...
            <Link to="/blog">
              <button className="group inline-flex items-center gap-2 px-5 py-2.5 bg-linear-to-r from-orange-500 to-orange-600 text-white rounded-xl font-medium transition-all duration-300 hover:-translate-y-0.5">
                عرض الكل
                <FaChevronLeft />
              </button>
            </Link>
          </div>
            <div className="space-y-10 bg-amber-950 g ">
            {featuredPosts.map((article, index) => (
              <Link key={index} to={`/blog/${article.slug}`}>
                <article className="group relative bg-[#161616] rounded-3xl overflow-hidden border border-[#262626] hover:border-orange-500/30 transition-all duration-500 cursor-pointer">
            ...
            >
             <input
              ...
              />
                          ...
                          <button
                type="submit"
                className="px-8 py-4 bg-linear-to-r from-orange-500 to-orange-600 text-white font-semibold rounded-xl hover:from-orange-600 hover:to-orange-700 transition-all duration-300 hover:shadow-lg hover:shadow-orange-500/50"
              >
                اشترك الآن
              </button>
            </form>
